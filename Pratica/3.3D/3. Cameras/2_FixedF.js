...
function main() {
  ...
  var colotAttributeLocation = gl.getAttribLocation (program, "a_color");
  ...
  setColors(gl);
  gl.enableVertexAttribArray(colotAttributeLocation);
  // Tell the attribute how to get data out of the colorBuffer
  var size = 3;
  var type = gl.UNSIGNED_BYTE;
  var normalize = true;
  var stride = 0;
  var offset = 0;
  gl.vertexAttribPointer(colotAttributeLocation, size, type, normalize, stride, offset);
  ...
  function drawScene() {
    ...
    // Computa a posição do Primeiro F
    var fPosition = [radius, 0, 0];     // Target

    // Usa matriz matemática para calcular a posição do Circulo
    var cameraMatrix  = m4.yRotation(cameraAngleRadians);
    cameraMatrix      = m4.translate(cameraMatrix, 0, 50, radius * 1.5);

    // Obtém a posição da Câmera da Matriz Computada
    var cameraPosition = [
      cameraMatrix[12],
      cameraMatrix[13],
      cameraMatrix[14],
    ...
    // Computa a Matriz da Camêra com Look at
...
function subtractVectros (a,b) {
  return [a[0] - b[0], a[1] - b[1], a[2] - b[2]];
}

function normalize(v) {
  var length = Math.sqrt(v[0] * v[0] + v[1]*v[1] + v[2]*v[2]);
  // Garante que não divide por 0
  if (length > 0.0001) {
...
var m4 = {
  ...
  lookAt : function (cameraPosition, target, up) {
    var zAxis = normalize (
      subtractVectros(cameraPosition, target));
